...
const user_urls = async (logged_user: LoggedUser): Promise<LoggedUserUrls> => (
...
async function get_right_and_parents(logged_user: LoggedUser, id: string, self_attrs: MultiAttrs): Promise<[Right, SgroupOutAndRight[]]> {
    const user_urls_ = await user_urls(logged_user)
...
// sizeLimit is applied for (const each subject source, so the max number of results is sizeLimit * nb_subject_sources
