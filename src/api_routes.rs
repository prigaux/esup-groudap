...
async fn set_test_data(config: &State<Config>, user: LoggedUser) -> Json<bool> {
    to_json(async { test_data::set(&mut my_ldap::open(&config.ldap, &user).await?).await }.await)
}
#[get("/clear_test_data")]
async fn clear_test_data(config: &State<Config>, user: LoggedUser) -> Json<bool> {
    to_json(async { test_data::clear(&mut my_ldap::open(&config.ldap, &user).await?).await }.await)
}
#[get("/add_test_data")]
async fn add_test_data(config: &State<Config>, user: LoggedUser) -> Json<bool> {
    to_json(async { test_data::add(&mut my_ldap::open(&config.ldap, &user).await?).await }.await)
}
...
async fn create(id: String, kind: Option<&str>, attrs: Json<Attrs>, config: &State<Config>, user: LoggedUser) -> Json<bool> {
    ...
    to_json(async {
        api::create(&mut my_ldap::open(&config.ldap, &user).await?, kind, &id, attrs.into_inner()).await
    }.await)
}
...
async fn delete(id: String, config: &State<Config>, user: LoggedUser) -> Json<bool> {
    to_json(async {
        api::delete(&mut my_ldap::open(&config.ldap, &user).await?, &id).await
    }.await)
}
...
async fn modify_members_or_rights(id: String, mods: Json<MyMods>, config: &State<Config>, user: LoggedUser) -> Json<bool> {
    to_json(async {
        api::modify_members_or_rights(&mut my_ldap::open(&config.ldap, &user).await?, &id, mods.into_inner()).await
    }.await)
}
